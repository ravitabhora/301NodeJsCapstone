...
class RequestHandler {
	...
	throwError(status, errorType, errorMessage, errorDetail = '', res = '') {
		...
		return (e) => {
			...
			e.errorType = errorDetail ? {errorDetail} : errorType;
			throw e;
		};
	}
	...
	sendError(req, res, error) {
		...
		if(error.type == 'validationError') {
			const errors = error.errorType.errorDetail;
			return res.status(error.status || 500).json({
				type: 'error', message: error.message || error.message || 'Unhandled Error', errors,
			});	
		}
		return res.status(error.status || 500).json({
